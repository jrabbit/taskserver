...
tmpdir=$(mktemp -d -t taskd.get-orig-source.XXXXXX)
orig_dir="taskd-${version}+repack.orig"
git clone -b "$tag" --depth 1 https://github.com/GothenburgBitFactory/taskserver.git "$tmpdir/${orig_dir}"
...
> "../taskd_${version}+repack.orig.tar.xz"
rm -rf "$tmpdir"
